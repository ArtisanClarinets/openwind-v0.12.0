...
def setup_anim(write_to_file=False, l_ele=0.01, order=6, n_steps=200, ax=None):
    ...
    plt.xlabel("x")
    plt.ylabel("Pressure")
    ax.set_title(f"$\\Delta t = T/{n_steps}$, $h={l_ele:.2g}$")
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(1, 3, sharex=True, sharey=True, figsize=(12, 2.6))
    update_fcts = []
    for l_ele, n_steps, ax in zip(l_eles, nn_steps, axs):
        update_fct = setup_anim(write_to_file=write_to_file, l_ele=l_ele, order=order, n_steps=n_steps, ax=ax)
        update_fcts.append(update_fct)
    ...
    def next_frame(frame):
        ...
        for i in range(3):
            fct = update_fcts[i]
            n_steps = nn_steps[i]
            frame_i = round(frame * n_steps / 100)
            artists = artists.union(fct(frame_i))
        return artists

    anim = FuncAnimation(fig, next_frame, 100, interval=60)

    if write_to_file:  # Export the animation instead of showing it interactively?
        f = "Defense_4_anim_gauss4.gif"
